...
const Statisics = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto relative container px-4  mt-10">
            <div className="statisics-inner flex flex-col xl:flex-row xl:items-start xl:gap-10">
                <div className="flex justify-between items-start gap-x-6 md:gap-x-10 w-full xl:w-[45%]">
                    ...
                    <p className="text-start flex-1 text-[#A6A6A6] text-[15px] md:text-[18px] xl:text-[20px] font-medium leading-5">{t('statisics_info')}</p>
                </div>
                <ul className="flex flex-col w-full xl:w-[50%] space-y-8 xl:space-y-0 md:flex-row justify-between items-center text-center mt-10 xl:mt-0">
                    {statisicsInfo.map((el, i) => (
                        <li key={i+1} className="flex flex-col space-y-3">
                            <h2 className="text-[#1A5C7F] text-[40px] md:text-[] xl:text-[64px] xl:font-bold font-semibold leading-10 flex justify-center gap-1">{el.num} <p className="text-[35px] md:text-[42px]">+</p></h2>
